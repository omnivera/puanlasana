...
<th class="text-center" style="width:22vw">Platform</th>
...
<tr v-for="(veri,index) in platformlarold" :key="veri.id" >
...
<td class="text-left">  <div >
  <div >
  <input type="search" style="background:green;color:white" autocomplete="off"  class="form-control" v-model="veri.platformad" id="exampleFormControlInput1" >
</div>
</div>
</td>
...
<tr v-for="(veri,index) in platformlar" :key="veri.id" >
...
<td class="text-left">  <div >
  <div >
  <input type="search" autocomplete="off" style="background:yellow;"  class="form-control" v-model="veri.platformad" id="exampleFormControlInput1" >
</div>
</div>
</td>
...
export default {
 ...
 const veriduzenle= (veri)=>{
   ...
   platformlarold.value=[]
    platformlar.value=[]
        firestoreRef.collection('kategoriler').doc(veriID.value).collection('platformlar').get()
        .then(snapshot =>{
            if (snapshot.size > 0) {
                  ...
                  snapshot.forEach(doc => {
                   ...
                   platformlarold.value.push({...doc.data(),id:doc.id})
  ...
  const altkatsil=async (altkat,index)=>{
            ...
            await firestoreRef.collection('kategoriler').doc(veriID.value).collection('platformlar').doc(altkat.id).delete()
             platformlarold.value.splice(index,1)
 ...
 firestoreRef.collection('iteminfo').doc('eHjkoM7nWcs8ya8kQTmX').update({
                   ...
                   platformlarcount: doc.data().platformlarcount - 1 ,
  ...
  const satirekle= ()=>{
            ...
            platformlar.value.push({platformad:""})
        }

          const satirsil= (index)=>{
          
            platformlar.value.splice(index,1)
        }

          const silbtnkontrol=computed(()=>{

     
            if (platformlar.value.length==1) {
              return true
            }
...
kisim.value=""
  ...
  platformlarold.value.forEach(veri=>{


 firestoreRef.collection('kategoriler').doc(veriID.value).collection('platformlar').doc(veri.id).update({

                  platformad:veri.platformad,
    ...
    platformlar.value.forEach(veri=>{
...
const res = firestoreRef.collection('kategoriler').doc(veriID.value).collection('platformlar').doc(veri.id).set(data);
 ...
 firestoreRef.collection('iteminfo').doc('eHjkoM7nWcs8ya8kQTmX').update({
                   ...
                   platformlarcount: doc.data().platformlarcount + platformlar.value.length ,
...
setTimeout(  function(){
    ...
    gbasarili.value=false
          ...
          return {kisim,verikayit,veriler,veriduzenle,guncellekisim,veriguncelle,verisil,basarili,basarisiz,kisimgoster,gbasarili,gbasarisiz,platformlar,satirekle,satirsil,
          silbtnkontrol,platformlarold,altkatsil
        }
